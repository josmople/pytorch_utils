def lazyload_submodules(root_modname=None, include_submodules=None, exclude_submodules=None, stack_idx=1) -> True:
    ...
    # if load_lazy_submodules(): # returns False (even if it says true) so the following code will not be executed
    #     from . import submodule # will be registered by intellisense (but not actually loaded)
